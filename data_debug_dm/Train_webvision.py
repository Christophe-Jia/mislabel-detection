...
def warmup(epoch,net,optimizer,dataloader,training_dynamics):
    net.train()
    ...
    for batch_idx, (inputs, labels, index) in enumerate(dataloader):      
        ...
        for b in range(inputs.size(0)):
            idx = index[b].item()
            idx_probs = training_dynamics.get(idx, [])
            idx_probs.append(logits[b])
            training_dynamics[idx] = idx_probs
...
def eval_train(model,all_loss,training_dynamics):    
    model.eval()
    ...
    return prob,all_loss,training_dynamics
...
if args.repair_ratio == 0.0:
    ...
    stats_log=open('./web_checkpoint/%.1f_repair'%(args.repair_ratio)+'_stats_%s.txt'%time_stamp,'w') 
    test_log=open('./web_checkpoint/%.1f_repair'%(args.repair_ratio)+'_acc_%s.txt'%time_stamp,'w') 
    ...
    noise_file='./%s/repair%.2f.json'%(args.dataset,args.repair_ratio)
    loader = dataloader.webvision_dataloader(batch_size=args.batch_size,num_workers=5,root_dir=args.data_path,log=stats_log, num_class=args.num_class,noise_file=noise_file)
...
for epoch in range(args.num_epochs+1):   
    ...
    if epoch<warm_up:       
        ...
        print('Warmup Net1')
        training_dynamics[0] = warmup(epoch,net1,optimizer1,warmup_trainloader,training_dynamics[0])    
        print('\nWarmup Net2')
        training_dynamics[1] = warmup(epoch,net2,optimizer2,warmup_trainloader,training_dynamics[1]) 
    ...
    prob1,all_loss[0],training_dynamics[0]=eval_train(net1,all_loss[0],training_dynamics[0])   
    print('\n==== net 2 evaluate training data loss ====') 
    prob2,all_loss[1],training_dynamics[1]=eval_train(net2,all_loss[1],training_dynamics[1])
    
    if web_acc[0]>best_acc:
        torch.save(all_loss,checkpoint+'model%s.pth.tar'%(args.id)) 
        best_acc = web_acc[0]
